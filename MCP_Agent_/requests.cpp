...
void Requests::apiCall(QObject *parent, const QString &host, const QString &path,
    ...
    QObject::connect(socket, &QSslSocket::disconnected, socket, [socket, callback]() {
        ...
        if (parseError.error != QJsonParseError::NoError) {
            ...
            << "\nHeaders:" << headerBlock
            << "\nBody (first 500 bytes):" << finalBody.left(500);
            callback(QJsonObject());
            socket->deleteLater();
            return;
        }
    ...
    QObject::connect(socket, &QSslSocket::errorOccurred, socket, [socket](QAbstractSocket::SocketError) {
        qWarning() << "Socket error:" << socket->errorString();
    });

    QObject::connect(socket, static_cast<void(QSslSocket::*)(const QList<QSslError> &)>(&QSslSocket::sslErrors),
                     socket, [socket](const QList<QSslError> &errors) {
                         for (const auto &e : errors) qWarning() << "SSL error:" << e.errorString();
                     });
